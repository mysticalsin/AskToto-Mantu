...
function StandSection({ data, now }: { data: DashboardData; now: number }) {
  const stand = useMemo(() => {
    try {
      return buildStandLayers(data, now)
    } catch (e) {
      ...
      return {
        deterministic: {
          ...
          answers: [],
          evidencePayload: {},
          deterministicHint: 'insufficient' as const
        },
        jev: { available: false, label: null, confidence: null, error: msg },
        explanation: { source: 'none' as const, text: null, citesDeterministic: true },
        suggestedActionPreviewRequired: true as const
      }
    }
  }, [data, now])
  ...
  return (
    <section
      className="mb-8 rounded-xl border border-[var(--color-mantu-border)] bg-[var(--color-mantu-surface)] p-5"
      data-metis-stand="1"
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2 text-[11px] font-semibold uppercase tracking-wide">
          <span className="rounded-full bg-white/10 px-2.5 py-1 text-white/70">
            Deterministic · {stand.deterministic.deterministicHint}
          </span>
          <span
            className={`rounded-full px-2.5 py-1 ${
              stand.jev.available ? 'bg-emerald-500/15 text-emerald-300' : 'bg-amber-500/15 text-amber-200'
            }`}
          >
            Jev ·{' '}
            {stand.jev.available && stand.jev.label ? stand.jev.label : 'decision assist unavailable'}
          </span>
      ...
      {stand.deterministic.answers.length === 0 ? (
        <p className="mt-3 text-sm text-rose-300" data-metis-stand-error="1">
          Stand snapshot failed{stand.jev.error ? `: ${stand.jev.error}` : '.'}
        </p>
      ) : (
                  ...
                  <li key={f.key + f.kind}>
                    {f.kind === 'missing' ? (
                      <span className="text-amber-200/90">
                        {f.label}: insufficient ({f.reason}) - not 0
                      </span>
                    ) : f.kind === 'count' ? (
                      <span>
                        {f.label}: <strong className="text-white/80">{f.value}</strong>
                      </span>
                    ) : (
                      <span>
                        {f.label}: {f.items.join(', ')}
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {a.evidenceLinks.map((l) => (
                  <Link
                    key={l.href + l.label}
                    to={l.href}
                    className="text-[11px] text-mantu-light underline-offset-2 hover:underline"
                  >
                    {l.label}
                  </Link>
                ))}
...
export function BriefingView({ data }: Props) {
  ...
  if (brainLooksEmpty) {
    ...
    // Cap3: stand must still render on empty brain (missing ≠ 0 / insufficient) — do not hide it behind EmptyState.
    return (
      <div className="mx-auto max-w-7xl px-6 py-8">
        <StandSection data={data} now={now} />
        <EmptyState
          title="Today"
