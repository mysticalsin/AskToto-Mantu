...
The exclusive window chrome stays opaque Mantu purple (`#3A0B6B`) so Mac simple-fullscreen is not a black void. **No rotating purple stripe wash** — `.onboard-stripes` / `onboard-stripe-spin` are deleted. First paint is the lady+universe hero plus the start CTA. After Act 1 the only bed is KineticGrid. No `filter: blur` drifting orbs.
