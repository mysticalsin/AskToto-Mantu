...
- **Out of scope.** Island/Hide park sizes stay frozen (Hide 8×2, Island peek 132×15). Hide/Island wake on the top-edge watch strip.
...
Hide and island **rest at** `display.bounds.y` so the top-edge watch rect can hit. The watch rect is the full top-edge approach strip.

- **Hide/island watch rect.** `y = display.bounds.y` (0 on the built-in Retina). Width is the display `workArea.width` (full top edge, **not** a leftover 560 slab). Height is the reserved menu-bar / notch inset plus the first work-area row (`workArea.y`, typically 39) so approach from below hits. Cap is `HOVER_ISLAND_HEIGHT_MAX_PX` === `TEAMS_UNDER_ISLAND_Y` (40), exclusive of Teams mute — not a 44px leftover slab, not `HOVER_HIT_BAND_MAX_PX` 44, not an 80–120px pad. Left, camera, and right at Y≈0–12 **and** Y≈39 reveal. Teams mute / camera / share sit **under** the island (Y≈40 and `TEAMS_MEETING_CHROME_Y` 48) and must not hit. Cursor watch (`hoverWatchRestRect`) hit-tests this always-on strip. The parked **hide window** is not this strip. The idle Island peek stays the small top-center rest (`OVERLAY_ISLAND_PEEK` 132×15). Hide rest paint stays 8×2. A hidden LSUIElement window uses `showInactive` on reveal.
- **Hide park (invisible).** 1–8px fully transparent hairline (or click-through). Not a 44px tint, not a 103px stub, not a 560-wide glass chip. Apple Hide is gone until the pointer enters the island. `clampHeight` / `BAR_MIN_HEIGHT` 44 must not grow this rest after a display move — stay 8×2 on every display. Ultron CGWindowList **8×2 at y=39** with opaque Mantu purple `#3A0B6B` is FAIL. Hide rest is transparent at `bounds.y`, or window opacity 0 / off-screen rest, so no purple hairline under the menu bar. Hover watch still uses `hoverWatchRestRect`.
...
- **Path C.** When `workArea.y` is 0 on a notched display, hide/island still rest at `bounds.y`. Revealed chrome uses a strut (`menuBarHeight`, or 37px) so content clears the notch. The hover hit stays peek-tall (no inset), not a strut-tall strip. Bar still floats; never a fake notch on Windows.

Hide/island **rest** stays at `bounds.y` (the top edge). Revealed chrome sits at `islandSafeTop` / `workArea.y` (~39) so content is fully below the notch — macOS clamps there anyway. Cursor watch **stays** open if the pointer is on the top-edge strip **or** the already-revealed Métis bar. Do not fight the OS with `setBounds(y=0)` on the full bar (that hide/reveal loop is the live stutter). Windows: top edge of the display, **no fake notch**.
...
**Hide leave → park (Ultron `c74e389`).** Mouse away from the overlay (for example ~(900, 600)) must park Hide to 8×2 within **1–2s**, no Escape. Main cursor-watch `hide` arms `scheduleOverlayLeavePark` (`OVERLAY_LEAVE_PARK_MS` 800). Do not rely only on the renderer spring. `restoreBarWidth` must not send hover-true (that re-opened Ask after leave). `parkOverlayAfterHideSpring` must not refuse park because the hover latch is stuck. Re-hover top-edge: 880×120+ `hasAsk=true`, same answer still there. The tick is one pure step (`overlayWatchStep`): main parks on leave only after it has SEEN the OS cursor in the strip or on the bar (`osHoverSeen` latch). A reveal the renderer opened on its own (forced toast, typed input, Island peek mouseenter, a CDP-synthetic hover) is parked by the renderer's own leave / force-end path, never by main 800ms later. A refused ghost heal (pointer already in the strip) falls through to reveal. Main logs `[overlay-watch] reveal / leave / park` transitions to main.log for Ultron.

**Relaunch.** Start from Hide park 8×2. No Settings window. Kill the previous Ultron pid first. Top-edge left/camera/right must stay PASS (Ask+Settings, not Show Métis). Then dwell away ~1–2s and confirm 8×2.
...
Exclusive onboarding is an **opaque** hero/universe hold (`transparent: false`, `#05010A`). Mac `setSimpleFullScreen` on a transparent `BrowserWindow` composites as a dead black void (Totos-Mac 044c0f1, 3600×2338 RGBA(0,0,0,0)). First visible frame is the lady+universe hero or this hold — never `#3A0B6B` stripe wash, even for one second. After `onboardingDone` the overlay is transparent again. Replay re-enters opaque exclusive and halts Goldberg first.
...
**Your setup loading orb.** Downloading / idle transcription and downloading / starting on-device model are `SetupRowState 'loading'`, not `'action'`. They show the Jakub thinking-orb (`InlineOrb` / `AgentStatus` kind=loading). Real progress in (0, 1) becomes a determinate % next to the orb. Empty progress: orb only, no fake 0%. `'action'` / **needed** is user work only (Allow Microphone, disk full, Retry). Continue stays blocked only while ASR is genuinely not ready and the list does not already claim complete. A complete-looking setup with a dead muted Continue is FAIL.
...
The exclusive window chrome stays opaque hero hold (`#05010A`) so Mac simple-fullscreen is never a solid black void. **No rotating purple stripe wash** — `.onboard-stripes` / `onboard-stripe-spin` are deleted. First paint is the lady+universe hero plus the start CTA, or the matching `#05010A` hold until the clip is ready. Never `#3A0B6B`. After Act 1 the only bed is KineticGrid (`#7F00DA` lines / `#9A2BF0` nodes). No `filter: blur` drifting orbs.
...
Act 1 (welcome) plays a full-viewport muted looping video behind the Métis mark (`object-cover`, `object-position: center`, z-0; UI z-10). Clip: CloudFront `hf_20260429_115139_0fc6bd3d` (April 29, lady looking at space). That is the only space shot. Not March 19. Not the July 14 clip. Not the April 11 clip. No CSS `filter` on the `<video>`. A purple Mantu tint sits on the video. A very slight loop-safe Ken Burns (`transform: scale` only) may run on the video. Wordmark, Next, and the Tony Walteur chip ease in (`opacity` / `transform`) and sit on the **darker sky**. Not a Bloom or Axon landing page. If the video fails or motion is reduced, the `#05010A` hero hold stays (drop the video). **Leave Act 1: pause and unmount the hero video.** After that beat the only bed is KineticGrid (`docs/design/ONBOARDING-KINETIC-GRID.md`). **Métis** (mark + wordmark) lands and **stays**. The wordmark is static. **No scramble.** The tagline may fade in once. **Next** and the Tony Walteur byline use liquid glass (capped blur, inset highlight, gradient-border). Steal the technique, not Bloom copy.
