...
- **Materials.** Glass shell (fresnel rim + one specular kiss) around a quiet living core. The body must **read** `#4CA8E8` at idle, not murky teal. Constellation is sparse (56 points, two faint chord families) and sits on the shell, not a glitter fill inside the ball. Electrons are off at idle. Perspective, not a 2D radial fill. Reduced-motion still looks spherical (lit still frame, not a disc).
