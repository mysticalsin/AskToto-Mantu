...
- **Hide/island watch rect.** `y = display.bounds.y` (0 on the built-in Retina). Width is the display `workArea.width` (full top edge, **not** a leftover 560 slab). Height is the notch / camera housing only (`HOVER_ISLAND_HEIGHT_MAX_PX`, typically ≤32) — not a 44px menu-bar slab, not `HOVER_HIT_BAND_MAX_PX` 44, not an 80–120px pad. Left, camera, and right at Y≈0–12 reveal. Teams mute / camera / share sit **under** the island (Y≈40 and `TEAMS_MEETING_CHROME_Y` 48) and must not hit. Cursor watch (`hoverWatchRestRect`) hit-tests this strip. The parked **hide window** is not this strip. The idle Island peek stays the small top-center rest (`OVERLAY_ISLAND_PEEK` 132×15). Hide rest paint stays 8×2. A hidden LSUIElement window uses `showInactive` on reveal.
...
- **Path C.** When `workArea.y` is 0 on a notched display, hide/island still rest at `bounds.y`. Revealed chrome uses a strut (`menuBarHeight`, or 37px) so content clears the notch. The hover hit stays housing-tall (≤32), not a strut-tall strip. Bar still floats; never a fake notch on Windows.
...
When `onboardingDone` flips true, `exitExclusiveOnboardingStage` leaves exclusive fullscreen and parks the default **hide** rest (or island / bar if Settings already chose one). Never an **880×816** mid-flow card. Never a 120×50 pill for hide. Re-apply `setAlwaysOnTop(true, 'screen-saver')` (the level exclusive used). Hide parks a 1–8px transparent rest; `hoverWatchRestRect` still covers the top-edge strip (`y = display.bounds.y`, width = `workArea.width`, height = camera housing only). Island is the peek capsule at the same Y. Bar keeps `workArea.y + margin`. Auto-resize must not grow hide into a 44px/103px slab or 880×816. Then destroy the exclusive stage. Do not leave layer 0. Start the cursor watch when layout is hide/island.
