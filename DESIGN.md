...
1. **hide** (default). Fully hidden until the pointer is in the Mac Dynamic Island / menu-bar / notch strip (the actual top of the screen), then reveal down. Leave that top band hides. Windows: top-center of the display, **no fake notch**.
2. **island**. The always-visible peek capsule (may sit in the island / notch). Hover the same top strip expands down. Leave that top band returns to the peek. Island must never minimize to a second disk.
...
- **Hide/island watch rect.** `y = display.bounds.y` (0 on the built-in Retina). Height is the menu-bar / Dynamic Island / notch strip only (`workArea.y - bounds.y` or `menuBarHeight`, typically ~24–38, never an 80–120px pad into the window). Cap `HOVER_HIT_BAND_MAX_PX`. Teams mute / camera / share sit **under** the menu bar, not in the Mac island — those Y values must not hit. Width at least the notch (min ~220, cap ~560), top-center. Cursor watch hit-tests this rect. The parked **hide window** is not this strip. The idle Island peek stays the small top-center rest (`OVERLAY_ISLAND_PEEK` 132×15). Hide rest paint stays 8×2.
...
Hide and island: hover or click expands **down** from the rest strip to the full bar at `islandSafeTop` (below the notch). Reveal only when cursor Y is in the menu-bar / notch band (approximately Y=0 through `menuBarHeight` / notch bottom, typically ~24–38px, **not** 120). Leaving that top band hides again. No sticky reveal from a mid-window hover — a leftover Settings-tall or 80–120px window must not keep or trigger Métis. Teams meeting chrome must never reveal Métis. Leave collapses (`pointer-leave` → grace → hide or peek). Bar does not auto-collapse. Pushing the pointer **up** into the Dynamic Island must keep the bar open (smooth, no flicker).
...
When `onboardingDone` flips true, `exitExclusiveOnboardingStage` leaves exclusive fullscreen and parks the default **hide** rest (or island / bar if Settings already chose one). Never an **880×816** mid-flow card. Never a 120×50 pill for hide. Re-apply `setAlwaysOnTop(true, 'screen-saver')` (the level exclusive used). Hide parks a 1–8px transparent rest; `hoverWatchRestRect` still covers the notch strip (`y = display.bounds.y`, height = menu-bar / notch band only, typically ~24–38, never 80–120). Island is the peek capsule at the same Y. Bar keeps `workArea.y + margin`. Auto-resize must not grow hide into a 44px/103px slab or 880×816. Then destroy the exclusive stage. Do not leave layer 0. Start the cursor watch when layout is hide/island.
