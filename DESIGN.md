...
`createWindow` when `onboardingDone` + hide/island parks this rest rect immediately (same as exclusive exit). On darwin that constructor is `type: 'panel'` (`!onboardingLive`); exclusive onboarding stays type normal; Windows stays normal. Before `createWindow`, darwin loads `metis-noconstrain.node` (`installMacNoConstrain`) so `constrainFrameRect:toScreen:` returns the frame unchanged — packaged hardened-runtime cannot use `DYLD_INSERT_LIBRARIES` (that path is show-tree / `libmetis-noconstrain.dylib` only). Never boot at 880×84 and hope hug wins. Never rest as 880×816. Never hug hide down to 120px.
