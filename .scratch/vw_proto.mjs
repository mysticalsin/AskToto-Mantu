...
function round(d) { return Math.round(d * 100) / 100 }

for (const eps of [0.02, 0.022, 0.025, 0.028]) {
  const absArcs = decodeAbsoluteArcs(topo)
  const simplifiedArcs = eps > 0 ? absArcs.map((a) => visvalingamWhyatt(a, eps)) : absArcs
  ...
  const proj1152 = geoMercator().translate([576, 288]).scale(152.948)
  const path1152 = geoPath(proj1152)
  let totalPts = 0
  let d1152 = ''
  const entries = []
  for (const f of fc.features) {
    const dd = path1152(f) ?? ''
    if (!dd) continue
    entries.push({ id: String(f.id ?? 'x'), alpha2: 'XX', d: dd.replace(/-?\d+\.\d+/g, (m) => Number(m).toFixed(2)) })
  }
  const bytes1152 = Buffer.byteLength(JSON.stringify(entries), 'utf8')
  let totalArcPts = 0
  for (const a of simplifiedArcs) totalArcPts += a.length
  console.log('eps', eps, 'arcPts', totalArcPts, 'features', entries.length, '1152bytes', bytes1152)
}

// 520 variant check at eps=0.02
{
  const eps = 0.02
  const absArcs = decodeAbsoluteArcs(topo)
  ...
  console.log('520 @ eps=0.02, 1 decimal:', Buffer.byteLength(JSON.stringify(entries), 'utf8'), 'features', entries.length)
}
