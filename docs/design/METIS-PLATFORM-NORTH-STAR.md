...
Hover hit is the always-on top-edge approach strip (work-area-wide, menu-bar inset plus first work-area row ≈39, not 44, not a leftover 560 slab). Left, camera, and right at Y≈0–12 and Y≈39 reveal the **full Ask bar** (880 × at least 120, hasAsk=true), not a 120×44 Show Métis hug stub and not an 880×44 OverlayPeek with only Show Métis. Teams mute / camera / share at Y≈40 and `TEAMS_MEETING_CHROME_Y` 48 miss. Hide: mouse leave auto-hides without Escape; re-hover restores the same answer until a new Ask replaces it. Listening chrome uses the same leave / re-hover. Main cursor-watch on darwin and Windows is the reveal; renderer `mouseenter` is not enough. A hidden LSUIElement window showInactive on that path. Do not hunt tray Show/Hide.
