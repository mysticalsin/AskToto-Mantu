...
Side channels: screenshot capture + cache in main; calendar (MS Graph via MSAL, dormant until Azure provisioning) feeding a meeting-detect poller (osascript/System Events on macOS) → 6-second opt-in toast; tray with agenda; auto-updater (autoDownload + install-on-quit, pointed at GitHub Releases — `releaseType: release` is now set and enforced by the check:release preflight gate).
...
CI: typecheck/tests/audit/SBOM/secret-scan + a check:release feed gate — genuinely unusual for v0.1. Cliffs: dmg at 2,048,103,604 bytes vs GitHub's 2,147,483,648 hard limit; publish previously defaulted to draft releases the updater can't see (since fixed: `releaseType: release` + check:release gate); a modelless Windows build already shipped with no gate catching it; the >2GB NSIS installer has never been installed on real hardware; Electron 33 past EOL (since fixed: bumped to Electron 39 — Chromium 142 / Node 22, min macOS 12); CI re-downloads 2.1 GB of unpinned, unverified model weights and uploads 14.2 GB of artifacts per push; macOS builds are arm64-only; every release forces up to a 1.9 GiB user download (differential unproven); repo + build outputs live inside OneDrive sync.
...
7. **Supply-chain and currency debt.** Unpinned HF model fetches with no hash verification and a release train 4.6% under a hard external limit (the EOL Electron and draft-release items are since fixed: Electron 39, `releaseType: release` gated by check:release). The next routine bump can break the build or the update channel.
