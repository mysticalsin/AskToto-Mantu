...
| QA-024 | High | With a cloud key configured (e.g. Kimi) and Dust connected, clicking **Spotlight Ref** while the locked Dust agent could not answer (agent not in the connected workspace, expired Dust session, or stale saved id) silently returned an answer generated by the active cloud provider instead of the managed sales-reference agent. | Spotlight Ref forces `providerOverride: 'dust'` + `agentOverride`, so Dust is tried first — but on a pre-token Dust failure the generic failover waterfall (`pickFailover`) handed the request to any other keyed provider, pre-empting the reconnect-Dust message. | Suppress cross-provider failover for any `agentOverride`-pinned request via a pure `allowCrossProviderFailover(req)` predicate consulted inside `pickFailover` (covers both the retry-budget and pre-token seams); let the existing `dustAgentUnavailableMessage` surface instead. Drop the redundant `agentOverride` from the generic follow-up cascade so it keeps its designed Dust-down failover. | Fixed and regression-tested: `allowCrossProviderFailover` unit tests, an `index.ts`/`App.tsx` source contract (`pinned-agent-boundary.contract.test.ts`), and the unchanged `local-cloud-boundary` failover-line contract all pass. Behavior is a visible reconnect-Dust message on a Dust outage rather than a silent substitute answer. |
