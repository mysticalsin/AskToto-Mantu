...
| **Recipients (processors)** | Microsoft 365 / OneDrive (storage) — covered by Mantu's existing M365 DPA, verify scope covers this content class; the cloud LLM provider configured in Settings, if any (Anthropic, OpenAI, NVIDIA NIM, DeepSeek, Qwen, MiniMax, Kimi, OpenRouter, Groq, Mistral, Grok, Gemini, or a custom endpoint — only the one(s) actually selected receive data); Dust.tt (Q&A today; *planned — Phase 5* additionally as a read recipient of published pages via the OneDrive connector) |
