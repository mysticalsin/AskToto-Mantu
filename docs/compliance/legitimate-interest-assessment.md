...
| Local LLM option for summarization (no cloud egress at all) | Shipped as an opt-in provider through llama.cpp `llama-server` b9957 (macOS universal: Apple Silicon + Intel; Windows x64). The **Qwen3.5 0.8B default is bundled**, including its GGUF/projector pair (763,759,712 bytes), and can be enabled offline on supported hardware. Local AI is off by default and existing selections are preserved. **Optional Qwen3.5 4B** requires explicit selection and a first-use download (3,584,533,344 bytes) into the profile from a pinned immutable Hugging Face revision; an enabled existing selection can resume on later launches. Both bundled and downloaded files are size- and SHA-256-verified before use; a corrupt packaged default requires installer repair. Downloads carry no meeting content, and local inference has no cloud egress. Offline 4B deployments must pre-place its verified profile files — see [`docs/ENTERPRISE-DEPLOY-WINDOWS.md`](../ENTERPRISE-DEPLOY-WINDOWS.md) |
