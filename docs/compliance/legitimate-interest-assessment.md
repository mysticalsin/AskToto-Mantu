...
| Local LLM option for summarization (no cloud egress at all) | Shipped as an opt-in provider — Qwen3.5 0.8B through llama.cpp `llama-server` b9957, the sidecar bundled in the installer (macOS universal: Apple Silicon + Intel; Windows x64). The ~728 MB weights are **not** in the installer: the app fetches them once per user profile from a pinned immutable Hugging Face revision, size- and SHA-256-verified before use (`src/main/llm/local-model-download.ts`). That first-run fetch is the only network request the local path ever makes; inference itself has no egress. An installation with no outbound internet has no local LLM unless the weights are pre-placed by deployment tooling — see [`docs/ENTERPRISE-DEPLOY-WINDOWS.md`](../ENTERPRISE-DEPLOY-WINDOWS.md) |
