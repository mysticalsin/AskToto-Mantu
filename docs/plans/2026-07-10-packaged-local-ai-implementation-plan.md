...
The bytecode main never imports node-llama. `worker-controller.ts` forks the worker's logical `${app.getAppPath()}/out/local-ai-worker/index.mjs` path with `execPath: process.execPath`, `ELECTRON_RUN_AS_NODE=1`, empty `execArgv`, an IPC stdio channel, a random handshake nonce, parent PID, bounded typed messages, and no listening socket. The worker entry and all JavaScript/transitive ESM dependencies remain physically in `app.asar`; only approved target native bins are unpacked. Electron 39's RUN_AS_NODE mode executes the ESM entry directly from ASAR, while node-llama-cpp rewrites its native-bin path to `app.asar.unpacked`. The child receives an allowlisted environment that strips proxy values, `NODE_OPTIONS`, `NODE_PATH`, `ELECTRON_NO_ASAR`, Hugging Face/provider/repository/signing credentials, and other secrets. The worker exits on parent disconnect and writes no content logs. Add an app self-test dispatch using `METIS_SELFTEST_KIND=local-ai-native` and absolute `METIS_SELFTEST_OUTPUT`. As the first branch after `app.whenReady()`, before proxy/provider/window initialization, it calls the real controller; the worker loads the selected GGUF from the passed verified absolute path, creates a bounded context, generates five tokens, is force-killed mid-generation, respawns once, generates again, disposes, writes a strict result, and exits nonzero on failure before the overlay is created.

`scripts/smoke-packaged-local-ai.mjs --app <absolute-executable> --result <absolute-json>` first copies the complete `.app` or `win-unpacked` tree to a detached temporary directory outside the checkout, then launches that exact copied executable with those environment variables, a sanitized environment, and isolated `ASKTOTO_USERDATA`; host Node never imports node-llama. It waits with a hard timeout, validates result/model/backend/resources/worker/binding paths, requires every real path to remain inside the detached app, and verifies clean exit. This prevents a missing packaged ESM dependency from resolving accidentally from the checkout's ancestor `node_modules`. The self-test installs a test-only network-deny shim before engine initialization and fails if model code calls `fetch`, `http`, `https`, Electron `net`, or a remote model resolver.
...
- Compile the worker as ESM and keep `node-llama-cpp` and `lifecycle-utils` external. Keep the worker output plus node-llama's JavaScript/transitive ESM closure in `app.asar`; unpack only approved target native `bins/**`. Do not broadly prune `node-llama-cpp/llama/**`: version 3.19.0 imports release metadata there at module evaluation, and grammars/templates can be runtime-used. This follows node-llama-cpp's official Electron layout and resolver, which rewrites native `.asar/` bin paths to `.asar.unpacked/`.
...
- Scan compiled worker output before packing and its logical ASAR entries after packing for prompts, credentials, provider configuration, or secrets imports; no worker JavaScript may exist physically under `app.asar.unpacked`, and the worker contains only runtime/IPC glue. MAS builds are explicitly local-tier-disabled because App Sandbox/run-as-Node support is outside this release.
...
Add `build:local-ai-worker` as `tsc -p tsconfig.local-ai-worker.json` and run it before the Electron build. Every raw `electron-vite build` path in package scripts/installers must call the composed build so packaging cannot skip the worker. Pin the package postinstall policy to skip native build/download probing. Add `dist:unpacked:mac` as `electron-builder --mac --arm64 --dir --publish never` after normal build/stage gates and a symmetric native Windows command. A passing detached native Mac spike unblocks the remaining source tasks. The symmetric detached Windows spike is mandatory before release but does not deadlock source work while GitHub Actions is unavailable. CI must use Node >=22.12 for `@electron/fuses@2.1.3`, and each native job must fetch, stage, and offline-check its own target payload. If the packaged worker fails on either native target, execute the documented fallback for the shared architecture: remove main bytecode, convert main output to ESM, and rerun both native spikes; never move inference to a renderer/utility process.
