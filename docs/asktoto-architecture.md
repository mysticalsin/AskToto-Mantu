...
**Meeting detection auto-start.** *Removed.* An earlier build polled window titles against the calendar to auto-start Listen, but the detector was cut (an app-open-but-not-in-a-meeting false positive would auto-record with no active meeting — a consent/privacy risk), so the `autoStartOnMeeting` setting was removed entirely. Meeting *awareness* remains via the calendar-based reminder (a native notification ~1 minute before a scheduled event, `meetingNotifications` setting) — it never starts recording on its own; the user always starts Listen deliberately, and `RecordingConsentReminder` (below) is the on-screen signal whenever a session is active.
