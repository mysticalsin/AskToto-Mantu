...
**Auto-start Listen (foreground-watcher).** When Teams, Zoom, or Google Meet becomes the front window, Métis can start Listen without a hotkey. This is **not** the old `src/main/meeting-detect/` / Accessibility poller (that stay gone; see I.2). Detection is `src/shared/meeting-auto-start.ts` on snapshots from the existing `foreground-watcher` (macOS NSWorkspace bundle id, Windows title + process). Settings `autoStartMeetings` defaults ON; the runtime gate is `onboardingDone && recordingConsent`. One start per meeting session. No auto-stop. Calendar is not consulted. Wave 0 still removed `MeetingDetectedToast` and `customMeetingApps`.
...
**Accessibility is not requested — deliberately.** An earlier build requested it for window-title enumeration to power the old meeting-detect auto-start. That AX path stays gone. Auto-start Listen (B.2) uses `foreground-watcher` only (NSWorkspace / GetForegroundWindow), never `isTrustedAccessibilityClient`. `main/platform-perms.ts → getPlatformPermissions()` checks only `microphone` and `screenRecording`. An unexplained Accessibility prompt is exactly the kind of thing enterprise IT flags. This keeps I.1's "no use of Accessibility APIs" claim literally true.
