...
describe('events keep seat OS after a later ask ingest', () => {
  it('does not wipe darwin when an ask arrives without os', async () => {
    ...
    expect(realtime).toContain('class="rt-feed-ago"')
    expect(realtime).toMatch(/class="rt-feed-where">Longueuil, CA</)
  })
})
...
describe('realtime and map use live heartbeats, not leftover OpenPanel', () => {
  it('paints real seats on the pre-Shoey map and fleet, never shoe SKUs', async () => {
    ...
    expect(html).toMatch(/data-rt-loc-pane="country"[\s\S]*?<table class="rt-table">/)
    expect(html).toMatch(/data-rt-loc-tabs[\s\S]*?data-tab="region"/)
    expect(html).toContain('data-city="Longueuil"')
    const realtime = html.slice(html.indexOf('data-page="realtime"'), html.indexOf('data-page="sessions"'))
    expect(realtime).toContain('aria-label="Live world map"')
    expect(realtime).toContain('>Live feed<')
    expect(realtime).toContain('data-live-feed')
    expect(realtime).toContain('data-rt-live-strip')
    expect(realtime).toContain('Unique seats · last 30 min')
    expect(realtime).toContain('>Locations<')
