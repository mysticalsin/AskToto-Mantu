...
describe('product sidebar (#105)', () => {
  ...
  it('keeps LIVE chrome plus Licenses: Sessions, Keys, Settings', async () => {
    ...
    expect(html).not.toContain('#0c1220')
    expect(html).not.toContain('#1a1f2e')
    const settings = html.slice(html.indexOf('data-page="settings"'))
    expect(settings).toContain('class="card"')
    expect(settings).toContain('class="rule"')
    expect(settings).not.toContain('settings-card')
