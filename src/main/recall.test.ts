...
describe('recall — deleteMeeting', () => {
  ...
  it('accepts a bare basename (what the renderer sends) as well as a full path', async () => {
    const file = await saveMeeting(testSettings, meeting)
    // basename() so this genuinely exercises the bare-filename path on Windows too (split('/') would
    // leave the full backslash path here, silently retesting the full-path case instead).
    const bare = basename(file)
    const r = await deleteMeeting(bare)
    expect(r.ok).toBe(true)
    expect(existsSync(file)).toBe(false)
  })
