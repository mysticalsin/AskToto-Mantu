...
describe('right-edge overlay placement', () => {
  ...
  it('falls back to the established top-center geometry when a full sidecar cannot fit', () => {
    ...
    expect(rightEdgePlacementFits(narrow, 300)).toBe(true)
    expect(resolveOverlayPlacement('right-edge', narrow)).toBe('top-center')
    expect(resolveOverlayPlacement('right-edge', narrow, 300)).toBe('right-edge')
    ...
    expect(overlayPlacementPosition({ placement: 'right-edge', width: 300, height: 560, layout: 'dock', metrics: narrow, topMargin: 8 })).toEqual({ x: 488, y: 68 })
