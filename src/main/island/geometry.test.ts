...
describe('exclusive onboarding stage (never a mid-flow card)', () => {
  ...
  it('primary onboarding CTAs use onboard-cta (min 52×220) and Act 2 is full-bar Métis + Intelligence', () => {
    ...
    expect(experience).toMatch(/className="onboard-cta onboard-glass no-drag focus-ring"/)
    expect(experience).not.toMatch(/className="onboard-cta[^"]*fade-up/)
    ...
    expect(demo).not.toMatch(/className="onboard-cta[^"]*fade-up/)
