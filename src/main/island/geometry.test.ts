...
describe('DESIGN.md overlay contract', () => {
  ...
  it('lives at the repo root and names path A then path C; hide/island rest at bounds.y', () => {
    ...
    expect(design).toMatch(/applyParkBounds/)
...
describe('island reveal/collapse wiring (index.ts)', () => {
  it('restoreBarWidth grows height at the same topClamp Y; resizeTo pins that Y', () => {
    ...
    expect(index).toMatch(/function applyParkBounds/)
    ...
    expect(applyPark).toMatch(/w\.setBounds\(park, false\)/)
    expect(applyPark).toMatch(/getBounds\(\)\.y/)
    expect(applyPark).toMatch(/setAlwaysOnTop\(true, 'screen-saver'\)/)
    expect(applyPark).toMatch(/setVisibleOnAllWorkspaces\(true, \{ visibleOnFullScreen: true \}\)/)
    ...
    expect(exit).toMatch(/applyParkBounds\(win, park\)/)
    const mode = index.slice(index.indexOf('function setWindowMode'), index.indexOf('function ensureWindow'))
    expect(mode).toMatch(/applyParkBounds\(win, park\)/)
