...
describe('island reveal/collapse wiring (index.ts)', () => {
  it('restoreBarWidth grows height at the same topClamp Y; resizeTo pins that Y', () => {
    ...
    expect(index).toMatch(/function createHideParkWindow/)
    expect(index).toMatch(/enableLargerThanScreen: true/)
    expect(index).toMatch(/type: 'panel'/)
    ...
    expect(hideParkCtor).toMatch(/setAlwaysOnTop\(true, 'screen-saver'\)/)
    expect(hideParkCtor).toMatch(/visibleOnFullScreen: true/)
    expect(hideParkCtor).toMatch(/setHiddenInMissionControl/)
    expect(hideParkCtor).not.toMatch(/setSimpleFullScreen/)
