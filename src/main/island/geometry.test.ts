...
describe('island reveal/collapse wiring (index.ts)', () => {
  it('restoreBarWidth grows height at the same topClamp Y; resizeTo pins that Y', () => {
    ...
    // One pure step per tick; the OS-hover latch decides leave -> park (cursor-watch.test.ts).
    expect(index).toMatch(/overlayWatchStep\(\{/)
    expect(index).toMatch(/osHoverSeen: overlayCursorWatchHovering/)
    expect(index).toMatch(/overlayCursorWatchHovering = step\.osHoverSeen/)
    ...
    expect(index).toMatch(/overlayAllowsHugWidth/)
    expect(index).toMatch(/isIncompleteAskReveal/)
    expect(index).toMatch(/overlayRevealedContentHeight/)
...
describe('exclusive onboarding stage (never a mid-flow card)', () => {
  ...
  it('App fills the stage — OnboardingV2 is not wrapped in the overlapping Panel card', () => {
    ...
    // Scope the no-card rule to the branch that actually renders the experience. The gate also holds the
    // auth-timeout retry, which is a genuine small card and must stay one.
    const render = block.slice(block.lastIndexOf('return (', block.indexOf('<OnboardingV2')))
    expect(render).not.toMatch(/<Panel>/)
  ...
  it('primary onboarding CTAs use onboard-cta (min 52×220) and Act 2 is full-bar Métis + Intelligence', () => {
    ...
    const stageBlock = css.slice(css.indexOf('.onboard-stage {'), css.indexOf('}', css.indexOf('.onboard-stage {')))
    expect(stageBlock).toMatch(/#05010a|#05010A/)
    expect(stageBlock).not.toMatch(/#3a0b6b|#3A0B6B/)
    expect(stageBlock).not.toMatch(/#7f00da|#7F00DA/)
