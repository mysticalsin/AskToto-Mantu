...
describe('finding 5: cliPrimary is filtered by the org allowlist', () => {
  it('pickWorkingCliPrimary receives the org allowlist so a blocked CLI cannot win', () => {
    ...
    const routing = readFileSync(join(__dirname, '../shared/ask-routing.ts'), 'utf8')
    expect(routing).toMatch(/if \(input\.allowed && !input\.allowed\.includes\(id\)\) return false/)
  })
})
...
describe('emergency force quit remains available when the renderer is wedged', () => {
  it('registers Ctrl+Cmd+Esc on macOS outside user-configurable shortcuts and uses the normal bounded quit path', () => {
    ...
    // meeting is not lost. app.exit() must never be reachable ahead of app.quit().
    ...
    // Past that flush window a wedged renderer never answers and its unresponsive window can stop the
    // quit completing at all, which is the freeze this accelerator exists to escape. So the polite path
    // gets a bounded grace and then the process goes down regardless, killing the sidecars on the way
    // because app.exit() never emits will-quit. Behavioural proof: emergency-force-quit.test.ts runs
    // this function against a fake clock.
