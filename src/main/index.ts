...
function restoreBarWidth(): void {
  ...
  if (placement === 'right-edge' && layout === 'dock') {
    // Grow the panel around the sliver that is already on screen, instead of re-deriving a Y from the
    // same normalized value through a height-dependent range - which put the two rects in different
    // places and made the resting pill visibly jump the moment the dock opened.
