...
function auditWindows(files: { file: string; source: string }[]): WindowAudit {
  ...
  for (const { file, source } of files) {
    for (const match of source.matchAll(NEW_WINDOW)) {
      ...
      // while doing the opposite of what the audit exists to guarantee: the QA force-paint branch added
      // exactly such a call, and from that moment deleting the real construction-time call was invisible
      // to the audit. A literal `false` is not evidence of protection.
  ...
  beforeEach(async () => {
    ...
    ;(electron.BrowserWindow as unknown as ReturnType<typeof vi.fn>).mockImplementation(function () {
      ...
      const win = {
        ...
        // `intelWin.isMinimized is not a function` before reaching its own assertion.
