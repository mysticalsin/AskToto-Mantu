...
//     (`/Library/Application Support/AskToto`, `/etc/asktoto`) is root-owned, so a standard user
//     cannot forge it. On Windows the natural location `%ProgramData%\AskToto` inherits ProgramData's
//     default ACL, which grants `BUILTIN\Users` write — so ANY standard user (no elevation) can
//     pre-create `%ProgramData%\AskToto\managed-config.json` and have it consumed as authoritative IT
