...
export function setBidstackApiKey(key: string): void {
  ...
  if (useFileBackend()) {
    ...
    if (!safeStorage.isEncryptionAvailable()) {
      throw new Error(
        'Encryption is unavailable on this machine. AskToto cannot safely store the BidStack API key.'
      )
    }
  ...
  try {
    ...
    throw new Error(
      `Couldn't save the BidStack API key — AskToto can't write to its data folder${
