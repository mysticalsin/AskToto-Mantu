...
describe('tell the room — designed consent on personalize', () => {
  ...
  it('is one glass card on personalize, after the mode cards, before Continue', () => {
    ...
    expect(tell).toMatch(/margin-inline:\s*auto/)
    expect(tell).toMatch(/align-items:\s*center/)
    expect(tell).not.toMatch(/text-align:\s*left/)
    ...
    expect(css).not.toMatch(/rgba\(\s*255,\s*255,\s*255,\s*0\.48\s*\)/)
    expect(css).not.toMatch(/rgba\(\s*255,\s*255,\s*255,\s*0\.36\s*\)/)
