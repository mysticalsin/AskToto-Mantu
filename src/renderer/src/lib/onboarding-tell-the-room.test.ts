...
describe('tell the room — designed consent on personalize', () => {
  ...
  it('is one glass card on personalize, after the mode cards, before Continue', () => {
    ...
    expect(veil).toMatch(/at 50% 12%/)
    expect(veil).toMatch(/rgba\(\s*255,\s*255,\s*255,\s*0\.48\s*\)/)
    ...
    expect(act4Stage).toMatch(/at 50% 0%/)
    expect(act4Stage).toMatch(/rgba\(\s*255,\s*255,\s*255,\s*0\.36\s*\)/)
