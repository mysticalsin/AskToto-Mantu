...
describe('onboarding portal pill + mandatory tour', () => {
  ...
  it('opens as a slow soft pill, not a clip-path circle pop', () => {
    ...
    const stageRule = css.slice(css.indexOf('.onboard-stage {'), css.indexOf('@keyframes onboard-portal-open'))
    expect(stageRule).not.toMatch(/mask-size:\s*120px 36px/)
    expect(css).toMatch(/\.onboard-stage--portal-close[\s\S]*?mask-size:\s*280vmax 180vmax/)
  ...
  it('tour is mandatory: no Skip path, and finish still requires recordingConsent', () => {
    ...
    expect(experience).not.toMatch(/Skip to the end/)
