...
export function App(): JSX.Element {
  ...
  const requireProvider = useCallback((): boolean => {
    ...
    return false
  }, [settings?.providerReady, settings?.hasApiKey, settings?.provider, openSettings])
  ...
  const factCheck = useCallback(() => {
    ...
    // The router (chooseQuickActionRoute) sends us down this text branch whenever a transcript lingers,
    // even with empty input and listening already stopped (e.g. right after a meeting ends) — without this
    // fall-through the claim/listening checks above all miss and Ctrl+Shift+F silently does nothing.
    if (transcript.trim()) {
      ask.run({
        mode: 'answer',
        kind: 'factcheck',
        label: 'the conversation so far',
        prompt: buildFactCheckClaimPrompt(transcript)
      })
    }
  }, [
    input,
  ...
  const copilotBody = useMemo(
    ...
    [listen.lines, suggest.answer, mode, listen.listening, listen.loading, listen.loadingPct, listen.error, captureError, autosaveWarn, transcriptShown, endReview]
  )
  ...
  }, [pastMeeting, ask.answer, listen.lines, savedPath, saveError, saveAttempts, settings?.showFullTranscriptInReview, settings?.bidstackConnected, settings?.bidstackTools, followup.answer, generateFollowup, retryAnswer, manualSave, resumePastMeeting, openPastMeeting, reset, recapSkipped, openSettings])
