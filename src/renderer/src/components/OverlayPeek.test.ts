...
describe('OverlayPeek hide rest is invisible', () => {
  it('hide is a transparent hairline; island keeps the visible peek', () => {
    ...
    expect(app).toMatch(/function parkOverlayAfterHide\(force = false\): void \{\s*void window\.toto\.parkAfterHide\(force\)\.catch\(\(\) => \{\}\)/)
    expect(app).toContain('const parkCurrentOverlayAfterHide = useCallback')
