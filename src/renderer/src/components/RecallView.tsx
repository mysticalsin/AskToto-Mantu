...
export function RecallView({
   ...
   *  void — openMeeting below surfaces it via that row's rowErrors entry so a failed open is never
   *  silently swallowed. */
  ...
  const importAudio = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      // A mid-import failure may still have saved a PARTIAL meeting on the main side (see
      // handleImportChunk's recovery path) — refresh so it shows up here instead of only being
      // discoverable after some unrelated refetch, with nothing on screen to confirm the error message's
      // "saved what was transcribed so far" claim.
      refreshList()
    } finally {
      unsub?.()
    }
   ...
   *  silently doing nothing — show it as this row's inline error, same per-file banner rename/delete use. */
      ...
      if (onOpenMeeting) {
        ...
        if (result instanceof Promise) {
          void result
            ...
            .catch((e) => flagRowError(f, e instanceof Error ? e.message : 'Could not open this meeting.'))
        }
      } else {
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="mb-2 flex items-center gap-2">
        ...
        <TextButton
          ...
          title="Import an audio recording and transcribe it on-device (Parakeet engine — optimized for major European languages)"
        >
