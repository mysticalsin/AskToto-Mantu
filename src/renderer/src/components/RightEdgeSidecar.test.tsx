...
describe('right-edge dock', () => {
  ...
  it('keeps a narrow accessible rail and a single fixed-composer body scroller without mounting Bar', () => {
    ...
    expect(css).toMatch(/\.right-edge-sidecar__chat-input:focus-visible \{[\s\S]*?outline:\s*2px solid/)
    expect(css).toMatch(/\.right-edge-sidecar__chat-input:focus-visible \{[\s\S]*?outline-offset:\s*2px/)
    ...
    expect(app).toContain("reduceRightEdgeDismissalLock(rightEdgeDismissalLockRef.current, 'explicit-close')")
    expect(app).toContain('shouldIgnoreRightEdgeNativeHover(rightEdgeDismissalLockRef.current, d.restoredFromParkedRail)')
    expect(app).toMatch(/reduceRightEdgeDismissalLock\(\s*rightEdgeDismissalLockRef\.current,\s*'native-hover-restored'\s*\)/)
    expect(app).toMatch(/const onOverlayPointerEnter = useCallback\(\(\) => \{[\s\S]*?reduceRightEdgeDismissalLock\([\s\S]*?'renderer-pointer-enter'/)
    expect(app).toMatch(/const onOverlayPointerLeave = useCallback\(\(\) => \{[\s\S]*?reduceRightEdgeDismissalLock\(rightEdgeDismissalLockRef\.current, 'renderer-pointer-leave'\)/)
  ...
  it('reconciles a native cursor-watch reveal with the parked right-edge renderer', () => {
    ...
    expect(cursorHover).toContain("'native-hover-restored'")
  ...
  it('guards a dirty recap before it launches and collapses for the standalone Intelligence dashboard', () => {
    ...
    expect(app).toMatch(/const revealOverlay = useCallback\(\(\) => \{[\s\S]*?reduceRightEdgeDismissalLock\(rightEdgeDismissalLockRef\.current, 'explicit-reveal'\)[\s\S]*?setRightEdgeDockDismissed\(false\)/)
  })
