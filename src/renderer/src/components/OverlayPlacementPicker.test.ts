...
describe('overlay physical placement controls', () => {
  ...
  it('makes the choice available from Settings and the guided onboarding flow', () => {
    ...
    expect(onboarding).toMatch(/ONBOARDING_PLACEMENT_HEADING/)
    expect(onboarding).toMatch(/data-onboard-appearance-step/)
    expect(onboarding).toMatch(/ONBOARDING_PLACEMENTS/)
