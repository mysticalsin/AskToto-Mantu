...
describe('AgentStatus composition', () => {
  it('renders thinking, working (processing), and planning orbs with the contracted states', () => {
    const thinking = html(<AgentStatus kind="thinking" size="hero" />)
    expect(thinking).toContain('Thinking')
    expect(thinking).toContain('data-agent-status="thinking"')
    expect(thinking).toContain('data-orb-state="solving"')
    const processing = html(<AgentStatus kind="working" size="hero" />)
    expect(processing).toContain('Thinking')
    expect(processing).toContain('data-agent-status="working"')
    expect(processing).toContain('data-orb-state="working"')
    const planning = html(<AgentStatus kind="planning" size="hero" />)
    expect(planning).toContain('Planning')
    expect(planning).toContain('data-agent-status="planning"')
    expect(planning).toContain('data-orb-state="shaping"')
  })
