...
describe('BAR-PILL contract', () => {
  ...
  it('never flattens: ray-sphere glass, same scale on X and Y', () => {
    ...
    expect(orb).toMatch(/fresnel/)
    expect(orb).toMatch(/spec = pow/)
    expect(orb).toMatch(/caustic/)
  ...
  it('does not pull Three.js or Spline from the network', () => {
    ...
    expect(orb).not.toMatch(/float flare/)
    expect(orb).not.toMatch(/atan\(vUv\.y, vUv\.x\)/)
    expect(orb).toMatch(/float bloom = exp\(h/)
  })
  ...
  it('QUALITY hats: idle bar has no orb rAF; frame loop does no layout or GL lookups', () => {
    ...
    expect(orbBtn).toMatch(/aw-orb__rec/)
    expect(orb).toMatch(/REC_DOT_COLOR = 0xf0717a/)
    expect(orb).not.toMatch(/idle: 0xf0717a/)
  })
})
