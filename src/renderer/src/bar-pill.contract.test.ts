...
describe('BAR-PILL contract', () => {
  ...
  it('uses the real thinking-orbs package, not WebGL or a Fit Studio core', () => {
    ...
    expect(orbBtn).toMatch(/BrandThinkingOrb/)
    expect(orbBtn).not.toMatch(/from 'thinking-orbs'/)
    expect(brandOrb).toMatch(/from 'thinking-orbs'/)
    expect(brandOrb).toMatch(/MODE_DRAWS/)
    expect(brandOrb).toMatch(/resolvePreset/)
  ...
  it('Listen chrome must not squash the left Settings M', () => {
    ...
    expect(bar).toMatch(/data-bar-toolbar/)
    expect(bar).toMatch(/aw-toolbar__tools/)
    expect(bar).not.toMatch(/grid-cols-\[minmax\(30px,1fr\)_auto_minmax\(30px,1fr\)\]/)
    expect(bar).not.toMatch(/grid-cols-\[1fr_auto_1fr\]/)
    expect(bar).not.toMatch(/w-\[100px\]/)
