...
export type ModeSkillId = BuiltinMode | typeof HUMANIZER_SKILL_ID
...
export function expectedSkillIds(): ModeSkillId[] {
  return [HUMANIZER_SKILL_ID, ...CONVERSATION_MODES]
}
...
export function composeLockedSkillsAppendix(
  ...
  loaded: { modeSkill: LoadedSkill | null; humanizer: LoadedSkill }
): string {
  const parts: string[] = []
